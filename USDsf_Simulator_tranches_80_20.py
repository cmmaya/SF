...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 19 17:58:23 2025
...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 19 17:52:42 2025
...
# -*- coding: utf-8 -*-
"""
Production-Grade USDsf Stability Simulation with Tranche Logic

This script simulates the stability mechanism for USDsf tokens using a Monte Carlo approach,
now extended to incorporate normal vs. preferential tranches in each trove, where preferential
tranches only get redeemed after normal tranches are exhausted.
"""
...
class USDsfStabilitySimulation:
    def __init__(self, simulation_id=0):
        ...
        # NEW: Keep track of global normal vs. pref redemption amounts
        self.cum_normal_redeemed = 0.0
        self.cum_pref_redeemed = 0.0
        self.cum_normal_issued = 0.0
        self.cum_pref_issued = 0.0

    def fil_gbm_simulation(self, granularity="1h", periods=365):
        """
        Fetch or simulate FIL data. (Mocked GBM in this snippet)
        """
        n_steps_sim = periods
        dt = 1 / n_steps_sim
        mu = self.drift_GBM
    ...
    def run(self):
        ...
        Returns: (data, total_liquidations, final_liquidity, final_Price_USDsf)
        """
        ...
        def liquidate_troves(troves, idx, data, price_fil_current):
            troves = troves.copy()
            troves['CR_current'] = troves['fil_Price'] * troves['fil_Quantity'] / (troves['normal_supply'] + troves['pref_supply'])
            if idx - 1 < 0:
            ...
            debt_liquidated = (troves_liquidated['normal_supply'] + troves_liquidated['pref_supply']).sum()
        ...
        def adjust_troves(troves, idx):
            ...
            total_supply = troves['normal_supply'] + troves['pref_supply']
            check = (troves['CR_current'] - troves['CR_initial']) / (troves['CR_initial'] * troves['Rational_inattention'])

            # Troves that want to decrease supply
            ...
            # Re-scale normal/pref proportionally
            old_total = total_supply[mask_low]
            scaling_low = new_total_supply_low / old_total
            troves.loc[mask_low, 'normal_supply'] *= scaling_low
            troves.loc[mask_low, 'pref_supply']   *= scaling_low

            # Troves that want to increase supply
            ...
            issuance_increase = (new_total_supply_high - total_supply[mask_high]).clip(lower=0)
            issuance_USDsf_adjust += (rate_issuance * issuance_increase).sum()

            # Re-scale normal/pref proportionally
            scaling_high = new_total_supply_high / total_supply[mask_high]
            troves.loc[mask_high, 'normal_supply'] *= scaling_high
            troves.loc[mask_high, 'pref_supply']   *= scaling_high

            # Troves that want to adjust collateral instead
            mask_collat = (p < ratio) & ((check < -1) | (check > 2))
            # We do not change the supply distribution here; we adjust the collateral
            troves.loc[mask_collat, 'fil_Quantity'] = (
                troves.loc[mask_collat, 'CR_initial'] * (troves.loc[mask_collat, 'normal_supply'] + troves.loc[mask_collat, 'pref_supply']) /
        ...
        def open_troves(troves, idx, price_USDsf_previous, price_fil_current):
            ...
            for _ in range(number_opentroves):
                ...
                total_supply_trove = price_fil_current * quantity_fil / CR_ratio

                # NEW: Split into normal vs. pref. 
                # For example, 80% normal, 20% pref.  Could also do random splits if desired.
                normal_part = 0.8 * total_supply_trove
                pref_part   = 0.2 * total_supply_trove

                issuance_USDsf_open += rate_issuance * total_supply_trove

                new_troves.append({
                    "fil_Price": price_fil_current,
                    "fil_Quantity": quantity_fil,
                    "CR_initial": CR_ratio,
                    "normal_supply": normal_part,    # NEW
                    "pref_supply": pref_part,        # NEW
                    "Rational_inattention": rational_inattention,
                    "CR_current": CR_ratio
                })

                # Track how many normal/pref minted globally
                self.cum_normal_issued += normal_part
                self.cum_pref_issued   += pref_part
        ...
        def price_stabilizer(troves, idx, data, stability_pool, n_open, price_fil_current):
            ...
            # total supply of USDsf = sum of normal + pref
            supply = (troves['normal_supply'] + troves['pref_supply']).sum()
            ...
            if price_USDsf_current > 1.1 + rate_issuance:
                ...
                # NEW: 80/20 split for new trove
                normal_part = 0.8 * supply_trove
                pref_part   = 0.2 * supply_trove
                self.cum_normal_issued += normal_part
                self.cum_pref_issued   += pref_part
                ...
                new_trove = {
                    ...
                    "fil_Quantity": quantity_fil,
                    "CR_initial": CR_ratio,
                    "normal_supply": normal_part,
                    "pref_supply": pref_part,
                    "Rational_inattention": rational_inattention,
                    "CR_current": CR_ratio
                }
            ...
            # Floor arbitrage (Redemptions)
            if price_USDsf_current < 1 - rate_redemption:
                shock_redemption = np.random.normal(0, 0.001)
                ...
                # NEW: Redeem from normal first, then pref
                amount_to_redeem = redemption_pool
                i = 0
                while i < len(troves) and amount_to_redeem > 0:
                    trove_normal = troves.at[i, 'normal_supply']
                    trove_pref   = troves.at[i, 'pref_supply']

                    if trove_normal > 0:
                        redeem_normal = min(trove_normal, amount_to_redeem)
                        troves.at[i, 'normal_supply'] = trove_normal - redeem_normal
                        amount_to_redeem -= redeem_normal

                        # Globally track how many normal tokens got redeemed
                        self.cum_normal_redeemed += redeem_normal

                    if amount_to_redeem > 0 and trove_pref > 0:
                        redeem_pref = min(trove_pref, amount_to_redeem)
                        troves.at[i, 'pref_supply'] = trove_pref - redeem_pref
                        amount_to_redeem -= redeem_pref

                        # Globally track how many pref tokens got redeemed
                        self.cum_pref_redeemed += redeem_pref

                    # Recompute trove CR after partial redemption
                    trove_supply_total = troves.at[i, 'normal_supply'] + troves.at[i, 'pref_supply']
                    if trove_supply_total <= 0:
                        # If fully redeemed, trove effectively is zero supply
                        # you could remove it from the list if you want:
                        troves.at[i, 'normal_supply'] = 0
                        troves.at[i, 'pref_supply']   = 0
                    troves.at[i, 'CR_current'] = (
                        troves.at[i, 'fil_Price'] * troves.at[i, 'fil_Quantity'] / max(trove_supply_total, 0.0000001)
                    )

                    n_redempt += 1
                    i += 1

            return price_USDsf_current, liquidity_pool, troves, issuance_USDsf_stabilizer, redemption_fee, n_redempt, redemption_pool, n_open
        ...
        initial_data = {
            ...
            "MC_LQTY": 0,
            "price_LQTY": float(price_LQTY_initial)
        }
        results.append(initial_data)

        # NEW: troves store normal_supply & pref_supply
        troves = pd.DataFrame(columns=[
            "fil_Price", "fil_Quantity", "CR_initial",
            "normal_supply", "pref_supply", "Rational_inattention", "CR_current"
        ])

        # Open initial troves
        troves, n_open, issuance_USDsf_open = open_troves(
            troves, 0, initial_data["Price_USDsf"], price_fil[0]
        )
        total_init_supply = (troves["normal_supply"] + troves["pref_supply"]).sum()
        initial_data["issuance_fee"] = issuance_USDsf_open * initial_data["Price_USDsf"]
        initial_data["supply_USDsf"] = total_init_supply
        initial_data["liquidity"] = 0.5 * total_init_supply
        initial_data["stability"] = 0.5 * total_init_supply
        ...
        for idx in range(1, T):
            ...
            # total supply = normal + pref
            total_supply = (troves['normal_supply'] + troves['pref_supply']).sum()
            stability_pool = stability_update(results[-1]["stability"], return_stability, idx, total_supply)

            (
                price_USDsf_current, liquidity_pool, troves,
                issuance_USDsf_stabilizer, redemption_fee,
                n_redempt, redemption_pool, n_open
            ) = price_stabilizer(troves, idx, df_results, stability_pool, n_open, current_price_fil)
            ...
            n_troves = troves.shape[0]
            supply_USDsf = (troves['normal_supply'] + troves['pref_supply']).sum()
        ...
        # NEW: Optionally, log out redemption probabilities at the end
        # Probability normal tokens get redeemed at all:
        normal_redeem_prob = 0.0
        if self.cum_normal_issued > 0:
            normal_redeem_prob = self.cum_normal_redeemed / self.cum_normal_issued

        pref_redeem_prob = 0.0
        if self.cum_pref_issued > 0:
            pref_redeem_prob   = self.cum_pref_redeemed / self.cum_pref_issued

        logger.info(f"Simulation {self.simulation_id} finished.")
        logger.info(f"   Normal redemption prob ~ {normal_redeem_prob:.4%}")
        logger.info(f"   Pref redemption prob   ~ {pref_redeem_prob:.4%}")

        # You might embed these probabilities into the final dataset
        data['normal_redemption_prob'] = normal_redeem_prob
        data['pref_redemption_prob']   = pref_redeem_prob
